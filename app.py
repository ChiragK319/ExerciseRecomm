...
# df['images'] = df['images'].apply(lambda x: [image.strip(" '") for image in x.split(", ")])
df['images'] = df['images'].apply(lambda x: [image.strip(" '") for image in x.strip("[]").split(", ")])


# Connect to MongoDB
client = MongoClient("mongodb+srv://chiragkaka319:Imeandmyself319@mpr-exercisere-commenda.k16bal8.mongodb.net/")
db = client["exercises"]
        ...
        for exercise_id in exercise_ids:
            ...
            if exercise_doc:
                if 'instructions' in exercise_doc:
                    ...
                    if isinstance(exercise_doc['instructions'], list):
                        # Convert list to string and join elements with ','
                        instructions_str = ','.join(exercise_doc['instructions'])
                        # Replace "\n" with "<br>" to add line breaks in the instructions
                        exercise_doc['instructions'] = instructions_str.replace('.,', '<br>')
                    else:
                        # If instructions is already a string, replace characters directly
